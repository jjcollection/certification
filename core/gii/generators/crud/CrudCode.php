...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveField($modelClass,$column)
	{
		...
		else
		{
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}', array('class'=>'form-control','style'=>'width:30%'))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength, 'class'=>'form-control','style'=>'width:30%'))";
			}
		}
	}
